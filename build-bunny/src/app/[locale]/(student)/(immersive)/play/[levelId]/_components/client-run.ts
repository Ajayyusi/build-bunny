...
import type { PlayerFeedback, PlayerPayload } from "./types";

/**
 * Client-side optimistic run: same engine, same generators, same checks as
 * the server — the animation starts instantly while the authoritative POST
 * is in flight. Nothing computed here is trusted for stars/XP/unlocks; the
 * server response reconciles the verdict afterwards.
 */
...
function primaryFeedback(perVariant: CheckFailure[][]): PlayerFeedback | null {
