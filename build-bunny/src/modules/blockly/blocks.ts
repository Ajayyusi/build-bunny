...
const LABELS: Record<BlockLocale, BlockLabels> = {
  en: {
    ...
    repeatUntilGoal: "repeat until I reach the goal %1 %2",
    if_: "if %1 do %2",
  ...
  ar: {
    ...
    repeatUntilGoal: "كرّر حتى أصل إلى الهدف %1 %2",
    if_: "إذا %1 نفّذ %2",
  ...
  const jsonDefs: Record<BunnyBlockType, Record<string, unknown>> = {
    ...
    bb_repeat: {
      ...
      args0: [
        ...
        // A row break: without it the label joins the statement input's row
        // and the mouth opens to the RIGHT of "times", pushing nested blocks
        // off the edge of a tablet-width workspace.
        { type: "input_dummy" },
