...
| `feat/finish-polish` | §5 Robo Bunny in every player, §6 worksheets, Arabic teacher notes, quality-review fixes | ready for review |
...
| Item | Status | Evidence / files |
|---|---|---|
| "Explain this block" / "Why did that fail?" / "Smaller hint" / "Similar example" | ✅ | `players/shared/RoboHelp.tsx`. Block coding: explain the selected block, why the run failed (located at the step and block), the tier-1 hint, a similar example played by the real engine. **Now in every player** (`feat/finish-polish`): code reading, ordering, AI simulations, ethics, teach-by-example and grouping get "Why was that wrong?" (per mistake type), the smallest hint and "What is this about?" (the level's idea from its concept tags); the failure banner opens it on "why". e2e `help.spec.ts` covers both kinds of player |
...
| Lesson objectives, curriculum mapping, teacher guides, printables | ✅ | **Curriculum** page (`/teach/curriculum`): every published level in play order with objective, mission, concepts (labelled in EN/AR), age band, minutes and teacher notes (**Arabic for all 100**, flagged for native review); printable. Suggested **CSTA K–12 alignment** per module with each standard described in words (visible, printable). **Printable worksheets** per module (`/teach/curriculum/worksheet/[moduleId]`): boards to write a program for (every board of multi-board levels; debugging levels print the broken program), code to read, steps to order (never printed in answer position); optional answer key on its own page. Staff only, published snapshots only; integration + isolation + unit tests |
...
| Read-only parent view + weekly summary | ✅ | teacher creates a private read-only **family link** on the student page (no parent accounts): 32-byte token, only its SHA-256 hash stored, shown once, 90-day expiry, one live link per child, revocable, audited. Public page `/family/[token]`: the week's finished levels, days played, stars, worlds + Powers, what they are learning now; never attempts, hints, flags or teacher notes; unknown/expired/revoked all read "not active"; noindex. Migration `20260924090000_family_links`; integration covers access, hashing, replace, revoke, expiry. Weekly **email** (Resend, 2026-09-25): the teacher adds one family address per child; the invitation names the school, not the child, and nothing is sent until the family confirms; Thursday send (Friday catch-up) via Vercel Cron, quiet weeks skipped, one-click stop, deleted on remove / disable / erase; migration `20260925090000_family_email`; integration `family-email.test.ts`. Live once `RESEND_API_KEY`, `FAMILY_EMAIL_FROM` and `CRON_SECRET` are set |

## §7 Accessibility, language, safety, performance

| Item | Status | Evidence / files |
|---|---|---|
| Keyboard, screen reader, focus, non-colour feedback | 🟡 | Every block reachable by keyboard/tap through the palette + toolbar (44 px, labelled), and adding one is announced; dialogs trap focus and land on "Next" rather than "Skip"; results are `role="alert"`/`status`; on/off never colour-only. Automated axe-core scan (`e2e/a11y.spec.ts`) of home, map, briefing, editor, Robo Bunny's panel, and the teacher guide, worksheet and class pages, EN + AR, fails on serious/critical findings (it found and we fixed low-contrast avatar initials, badges, matrix cells and chips). **Not done:** a person-run screen-reader pass (NVDA/VoiceOver); Blockly's own canvas is excluded from the scan (third-party) |
...
| Performance on school devices | 🟡 | Production build, emulated low-end tablet (4× CPU slowdown, 1.6 Mbps, 150 ms latency; `scripts/perf-tablet.mjs`): home 1.1 s cold / 0.5 s warm; level briefing 2.1 s cold / 1.1 s warm; block editor ready 0.1 s after the tap. Level player 136 kB first-load JS. **Not done:** measurements on a real low-end tablet and a real school network |
...
- **On branches:** `feat/finish-polish` (PR #15, stacked on #14): Robo Bunny in every player, printable worksheets, Arabic teacher notes and concept labels, wider accessibility scan with contrast fixes, a load-time script, and three rounds of fixes from a four-part quality review (security/privacy, client reliability, grading integrity, UX/i18n/accessibility).
- **What the quality review found and we fixed:** a block id could carry code into the server's generated program; counter levels passed without counting; "use a trick" passed with an empty trick; build-your-own mazes could be won with one block; a tablet's stale copy of a child's blocks could overwrite newer work; the offline outbox could lose a run queued mid-resend or file it under another child; tapping Check offline queued duplicate attempts; small reflection counts identified children; the class-mission count showed children how many were ahead; family links survived a lapsed licence or a disabled account and could not be revoked on a read-only licence; the projector showed names by default; Arabic plurals; ages shown to children. Each fix has a test where one was possible.
- **Recheck:** a verification pass on the fixes found that one "add 4" still passed counter levels, that some saves did not move the device copy's version (offline work could still be lost after a maze Build or a hidden-tab save), that a trick calling only itself counted as a call, and that a teacher signed in on a shared tablet made a child's queued run disappear. All fixed: counter levels now grade how many times Add actually RAN (Add 0 doesn't count), the page tracks the server's draft version across remounts and every save (maze designs included), and a pass moves the version too. Running the browser suite after that caught one more race (a retry and a reconnect resend sending the same run at once); concurrent sends now share one request.
- **Known limits:** a maze level's "use Repeat" requirement is satisfied by any Repeat, even an empty one (a secondary check: without Repeat the result is PARTIAL; the maze must still be solved either way); levels changed here (Set counter removed) would reject old saved drafts containing that block — harmless now because this content has not reached schools.
- **Tested:** lint, type-check, 817 unit/integration tests, full Playwright suite at three screen sizes (32 passed, 19 single-size skips), production build.
- **Still open:** weekly family email (needs an email provider — your choice); Arabic native review; physical iPad / Safari; person-run screen-reader pass; real-device performance; AI tutor (not built by design).
